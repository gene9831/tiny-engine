...
import { useApp, useBlock, useModal, useMessage, useMaterial } from '@opentiny/tiny-engine-meta-register'
      ...
      fetchGroupBlocksById({ groupId })
        .then((data) => {
          ...
          requestUpdateGroup({
            ...
            useMessage().publish({ topic: 'fetchResource', data: { isInit: false } }) // 添加区块分组，不需要重新init页面或者区块。
          })
        })
        .catch((error) => {
          message({
